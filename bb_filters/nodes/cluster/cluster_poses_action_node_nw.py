#!/usr/bin/env python3
# _nw variant of the action-based pose clustering node.
#
# Identical to ClusterPosesActionNode except it derives from ClusterPosesNodeNw
# (PoseArray subscribers) and exposes a distinct node + action name so it can run
# ALONGSIDE the original shared action server without colliding. The recluster
# step in bins.py targets "cluster_poses_nw".
...
class ClusterPosesActionNodeNw(ClusterPosesNodeNw):
    """ROS node that runs pose collection and clustering as an action.

    See ClusterPosesActionNode for the full behaviour; only the base class
    (array subscribers) and the node/action names differ.
    """

    def __init__(self) -> None:
        super().__init__("cluster_poses_action_node_nw")
        ...
        self.get_logger().info("Cluster Poses Action Server (nw) initialized")
