...
class GateStatusNode(Node):
    def __init__(self):
        ...
        pinger_data = (
            self.get_parameter("pinger_data").get_parameter_value().string_value
        )

        # taking some doa to average for best result
        self.declare_parameter("doa_cluster_size", 7)
        self.doa_cluster_size = (
            self.get_parameter("doa_cluster_size").get_parameter_value().integer_value
        )
        ...
        self.pub_status_ = self.create_publisher(
            Bool, "/asv4/robotx/pinger_gate_status", 10
        )

        self.service = self.create_service(
            SetBool, "/robotx/activate_pinger_gate_node", self.service_callback
        )
