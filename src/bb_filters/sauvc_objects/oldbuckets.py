    ...
    def process(self, bboxes: DetectedObjects) -> DetectedObjects:
        ...
        for det in front_buckets:
            ...
            if is_top_visible and is_bottom_visible:
                ...
                rospy.logwarn("top or side not visible")
        if len(bot_buckets) > 0:
            ...
            rospy.loginfo(f"{camera_depth}, {self.camera_infos.get_info(289).P[0]}, {(self.bucket_diameter / 2) / (camera_depth - (self.bucket_depth - self.bucket_height))}")
            for det in bot_buckets:
                try:
                    xc, yc, r, sigma = taubinSVD(np.array(det.contour).reshape(-1, 2))
                except:
                    continue
                if np.abs(r - est_circle_radius) < 300:
                    ...
                    rospy.loginfo(f"Bucket radius rejected: ({xc}, {yc}), {r} expect {est_circle_radius}")
            ...
            if len(id_centers) - offset >= self.num_buckets:
                ...
                if len(self.acoustics_vecs) > 1:
                    ...
                    for i, center in enumerate(cluster_centers):
                        ...
                        distances[(center[0][0], center[0][1])] = dist
                        rospy.loginfo(f"D1: {np.linalg.norm(v1 - np.diag(v1@v.T).reshape(-1, 1) * v, axis=1)} {center[0][0], center[0][1]}")
                ...
                if best_cluster is not None:
                    ...
                    rospy.loginfo(f"Dists: {distances}, cluster_centers: {[np.array([cluster_centers[idx][0][0], cluster_centers[idx][0][1]]) for idx in ids.values()]}")
